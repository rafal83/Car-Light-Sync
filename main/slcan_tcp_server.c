...
static void process_slcan_rx_data(slcan_client_t *client, const uint8_t *data, int len) {
  ...
  ESP_LOGD(TAG, "Received %d bytes from client (slot %d) - ignoring", len, (int)(client - clients));
...
static void slcan_accept_task(void *arg) {
  ...
  while (server_running) {
    ...
    // Create client task
    BaseType_t ret = xTaskCreate(slcan_client_task, "slcan_client", 4096, client, 10, &client->task_handle);
...
esp_err_t slcan_tcp_server_start(void) {
  ...
  // Create accept task
  server_running      = true;
  BaseType_t task_ret = xTaskCreate(slcan_accept_task, "slcan_accept", 4096, NULL, 11, &accept_task_handle);
