        ...
        if (level == 0) { // Bouton appuyé (pull-up, donc 0 = appuyé)
            if (!button_pressed) {
                ...
                if (press_duration >= RESET_BUTTON_HOLD_TIME_MS) {
                    ESP_LOGW(TAG,
                             "Bouton reset maintenu pendant %lu ms, factory reset!",
                             press_duration);
            ...
            if (button_pressed) {
                uint32_t press_duration = esp_log_timestamp() - press_start_time;
                ESP_LOGI(TAG, "Bouton reset relâché après %lu ms",
                         press_duration);
                button_pressed = false;
                reset_led_shown = false;
                // Restaurer l'état de la LED immédiatement si le bouton est relâché avant 5s
                if (press_duration < RESET_BUTTON_HOLD_TIME_MS) {
...
esp_err_t reset_button_init(void) {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Erreur configuration GPIO bouton reset: %s",
                 esp_err_to_name(err));
        return err;
    }
    ...
    BaseType_t task_created = xTaskCreatePinnedToCore(
        reset_button_task, "reset_button", 2048, NULL, 5,
        &reset_button_task_handle, tskNO_AFFINITY);
