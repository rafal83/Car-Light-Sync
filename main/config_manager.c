...
bool config_manager_init(void)
{
    ...
    if (loaded == 0) {
        ...
        if (active_profile_id >= 0 && active_profile_id < MAX_PROFILES) {
            ...
            ESP_LOGI(TAG,
                     "Auto night mode: %s (will respond to CAN events)",
    ...
    for (int i = 0; i < MAX_PROFILES; i++) {
        ...
        if (err == ESP_OK) {
            ...
            if (required_size != sizeof(config_profile_t)) {
                ESP_LOGW(TAG,
                         "Profile %d a une taille incompatible (%d vs %d) - ignoré",
                         i,
                         required_size,
    ...
    for (int p = 0; p < MAX_PROFILES; p++) {
        ...
        for (int e = 0; e < CAN_EVENT_MAX; e++) {
            if ((profiles[p].event_effects[e].profile_id == profile_id) && (profile_id != p)) {
                ESP_LOGW(TAG, "Cannot delete profile %d: used by event %d in profile %d", profile_id, e, p);
                return false;
            }
        }
    }
...
bool config_manager_process_can_event(can_event_type_t event)
{
    ...
    if (event_effect->enabled) {
        ...
        if (duration_ms > 0) {
            ESP_LOGI(TAG,
                     "Effet '%s' activé pour %dms (priorité %d)",
                     config_manager_get_event_name(event),
                     duration_ms,
                     priority);
        } else {
            ESP_LOGI(TAG, "Effet '%s' activé (permanent, priorité %d)", config_manager_get_event_name(event), priority);
        }
...
void config_manager_update(void)
{
    ...
    if (any_active) {
        ...
        if (full_active) {
            ...
            led_effects_set_dual_directional(&active_events[left_slot].effect_config,
...
bool config_manager_set_led_hardware(uint16_t led_count, uint8_t data_pin)
{
    ...
    ESP_LOGI(TAG, "Configuration matérielle LED sauvegardée: %d LEDs, GPIO %d", led_count, data_pin);
    return true;
}
