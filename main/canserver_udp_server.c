    ...
    for (int i = 0; i < MAX_CANSERVER_CLIENTS; i++) {
        if (clients[i].active) {
            if ((now - clients[i].last_ping_time) > timeout_us) {
                ...
                ESP_LOGI(TAG, "Client %s:%d timed out (no ping for >5s), removing",
                         ip_str, ntohs(clients[i].addr.sin_port));
...
static void canserver_rx_task(void *arg)
{
    ...
    while (server_running) {
        // Receive UDP packet
        int len = recvfrom(udp_socket, rx_buf, sizeof(rx_buf), 0,
        ...
        if (!client) {
            ...
            if (is_hello) {
                ...
                if (client) {
                    ...
                    ESP_LOGI(TAG, "New client registered: %s:%d (received %d bytes)",
                             client_ip, ntohs(client_addr.sin_port), len);
                } else {
                    ESP_LOGW(TAG, "Max clients reached, ignoring hello from %s:%d",
                             client_ip, ntohs(client_addr.sin_port));
                }
            } else {
                ESP_LOGW(TAG, "Ignoring packet from unknown client %s:%d (not a hello, len=%d): [%02x %02x %02x %02x %02x]",
                         client_ip, ntohs(client_addr.sin_port), len,
        ...
        for (int i = 0; i < MAX_CANSERVER_CLIENTS; i++) {
            if (clients[i].active) {
                ...
                while (retry_count < max_retries) {
                    sent = sendto(udp_socket, send_buf, send_len, 0,
                    ...
                    if (sent == send_len) {
                        ...
                        if (clients[i].frames_sent % 50 < frame_count) {
                            char ip_str[INET_ADDRSTRLEN];
                            inet_ntop(AF_INET, &clients[i].addr.sin_addr, ip_str, sizeof(ip_str));
                            ESP_LOGI(TAG, "Sent %u frames to %s:%d (batch: %d, size: %d bytes)",
                                     clients[i].frames_sent, ip_str, ntohs(clients[i].addr.sin_port),
                                     frame_count, send_len);
                        }
                        break;
...
void canserver_udp_broadcast_can_frame(int bus, const twai_message_t *msg)
{
    ...
    if (frame_rx_count % 100 == 0) {
        ESP_LOGI(TAG, "Received %u CAN frames for broadcast (bus %d, ID 0x%03X)",
                 frame_rx_count, bus, msg->identifier);
    }
    ...
    if (frame_buffer.count < MAX_FRAMES_PER_PACKET) {
        memcpy(&frame_buffer.data[frame_buffer.count * PANDA_MSG_SIZE],
               frame, PANDA_MSG_SIZE);
        frame_buffer.count++;
    }
