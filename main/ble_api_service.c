...
static const ble_uuid128_t ble_command_uuid = BLE_UUID128_INIT(
...
static const ble_uuid128_t ble_response_uuid = BLE_UUID128_INIT(
        0xdc, 0xa9, 0x4b, 0x6f, 0x82, 0xea, 0x30, 0xaa, 0x1b, 0x4c, 0xeb, 0x52, 0x0c, 0x99, 0xa0, 0x64);
    ...
    switch (event->type) {
        case BLE_GAP_EVENT_CONNECT:
            if (event->connect.status == 0) {
                ...
                ESP_LOGW(BLE_API_TAG, "Connexion BLE echouee, statut=%d", event->connect.status);
        ...
        case BLE_GAP_EVENT_DISCONNECT:
            ...
            notifications_enabled = false;
            incoming_length = 0;
            ESP_LOGI(BLE_API_TAG, "Client BLE deconnecte, raison=%d", event->disconnect.reason);
            {
                struct ble_gap_adv_params adv = {
                        .conn_mode = BLE_GAP_CONN_MODE_UND,
                        .disc_mode = BLE_GAP_DISC_MODE_GEN,
        ...
        case BLE_GAP_EVENT_SUBSCRIBE:
            ...
            ESP_LOGI(BLE_API_TAG, "Notifications BLE %s", notifications_enabled ? "activees" : "desactivees");
            break;
    }
    return 0;
}
...
static bool ble_send_notification(const uint8_t* data, size_t len)
{
    ...
    while (offset < len) {
        ...
        if (!om) {
            ESP_LOGE(BLE_API_TAG,
                     "Erreur allocation mbuf (chunk %d/%d, offset=%d)",
                     chunk_count + 1,
...
static void ble_on_sync(void)
{
    ...
    ESP_LOGI(BLE_API_TAG,
             "Adresse BLE: %02x:%02x:%02x:%02x:%02x:%02x",
             addr.val[5],
             addr.val[4],
             addr.val[3],
             addr.val[2],
...
esp_err_t ble_api_service_init(void)
{
    ...
    BaseType_t task_created = xTaskCreatePinnedToCore(
            ble_request_task, "ble_api_req", 12288, NULL, 7, &request_task_handle, tskNO_AFFINITY);
