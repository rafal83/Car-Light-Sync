...
static esp_err_t profiles_handler(httpd_req_t* req)
{
    ...
    cJSON_AddStringToObject(root, "active_name", (active_id >= 0) ? profiles[active_id].name : "None");
...
static esp_err_t profile_update_handler(httpd_req_t* req)
{
    ...
    if (success && config_manager_get_active_profile_id() == profile_id) {
        ...
        ESP_LOGI(TAG,
                 "Updated night mode settings (auto: %s, brightness: %d, current state: %s)",
...
static esp_err_t event_effect_handler(httpd_req_t* req)
{
    ...
    if (event && effect) {
        ...
        bool success = config_manager_set_event_effect(profile_id,
                                                       event->valueint,
                                                       &effect_config,
...
static esp_err_t simulate_event_handler(httpd_req_t* req)
{
    ...
    ESP_LOGI(TAG, "Simulation événement CAN: %s (%d)", config_manager_get_event_name(event), event);
...
static bool apply_event_update_from_json(config_profile_t* profile, int profile_id, cJSON* event_obj)
{
    ...
    effect_config.reverse = (event_type == CAN_EVENT_TURN_LEFT || event_type == CAN_EVENT_BLINDSPOT_LEFT);

    uint16_t duration = duration_json && cJSON_IsNumber(duration_json) ? duration_json->valueint
                                                                       : profile->event_effects[event_type].duration_ms;
    uint8_t priority = priority_json && cJSON_IsNumber(priority_json) ? priority_json->valueint
...
esp_err_t web_server_start(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_uri_t effect_uri = {
                .uri = "/api/effect", .method = HTTP_POST, .handler = effect_handler, .user_ctx = NULL};
        ...
        httpd_uri_t profiles_uri = {
                .uri = "/api/profiles", .method = HTTP_GET, .handler = profiles_handler, .user_ctx = NULL};
        ...
        httpd_uri_t profile_create_uri = {
                .uri = "/api/profile/create", .method = HTTP_POST, .handler = profile_create_handler, .user_ctx = NULL};
        httpd_register_uri_handler(server, &profile_create_uri);

        httpd_uri_t profile_delete_uri = {
                .uri = "/api/profile/delete", .method = HTTP_POST, .handler = profile_delete_handler, .user_ctx = NULL};
        httpd_register_uri_handler(server, &profile_delete_uri);

        httpd_uri_t factory_reset_uri = {
                .uri = "/api/factory-reset", .method = HTTP_POST, .handler = factory_reset_handler, .user_ctx = NULL};
        httpd_register_uri_handler(server, &factory_reset_uri);

        httpd_uri_t profile_update_uri = {
                .uri = "/api/profile/update", .method = HTTP_POST, .handler = profile_update_handler, .user_ctx = NULL};
        httpd_register_uri_handler(server, &profile_update_uri);

        httpd_uri_t profile_update_default_uri = {.uri = "/api/profile/update/default",
                                                  .method = HTTP_POST,
        ...
        httpd_uri_t event_effect_uri = {
                .uri = "/api/event-effect", .method = HTTP_POST, .handler = event_effect_handler, .user_ctx = NULL};
        httpd_register_uri_handler(server, &event_effect_uri);

        httpd_uri_t profile_export_uri = {
                .uri = "/api/profile/export", .method = HTTP_GET, .handler = profile_export_handler, .user_ctx = NULL};
        httpd_register_uri_handler(server, &profile_export_uri);

        httpd_uri_t profile_import_uri = {
                .uri = "/api/profile/import", .method = HTTP_POST, .handler = profile_import_handler, .user_ctx = NULL};
        ...
        httpd_uri_t ota_upload_uri = {
                .uri = "/api/ota/upload", .method = HTTP_POST, .handler = ota_upload_handler, .user_ctx = NULL};
        httpd_register_uri_handler(server, &ota_upload_uri);

        httpd_uri_t ota_restart_uri = {
                .uri = "/api/ota/restart", .method = HTTP_POST, .handler = ota_restart_handler, .user_ctx = NULL};
        httpd_register_uri_handler(server, &ota_restart_uri);

        // Route de simulation
        httpd_uri_t simulate_event_uri = {
                .uri = "/api/simulate/event", .method = HTTP_POST, .handler = simulate_event_handler, .user_ctx = NULL};
        httpd_register_uri_handler(server, &simulate_event_uri);

        httpd_uri_t stop_event_uri = {
                .uri = "/api/stop/event", .method = HTTP_POST, .handler = stop_event_handler, .user_ctx = NULL};
        ...
        httpd_uri_t effects_list_uri = {
                .uri = "/api/effects", .method = HTTP_GET, .handler = effects_list_handler, .user_ctx = NULL};
        httpd_register_uri_handler(server, &effects_list_uri);

        httpd_uri_t event_types_list_uri = {
                .uri = "/api/event-types", .method = HTTP_GET, .handler = event_types_list_handler, .user_ctx = NULL};
        ...
        httpd_uri_t events_post_uri = {
                .uri = "/api/events", .method = HTTP_POST, .handler = events_post_handler, .user_ctx = NULL};
