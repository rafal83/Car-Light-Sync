...
static void monitor_task(void* pvParameters)
{
    ...
    while (1) {
        ...
        if (now - last_print > pdMS_TO_TICKS(30000)) {
            ...
            ESP_LOGI(TAG, "=== Statut ===");
            ESP_LOGI(TAG,
                     "WiFi AP: %s (IP: %s, Clients: %d)",
                     wifi_status.ap_started ? "Actif" : "Inactif",
                     wifi_status.ap_ip,
                     wifi_status.connected_clients);

            if (wifi_status.sta_connected) {
                ESP_LOGI(TAG, "WiFi STA: Connecté à %s (IP: %s)", wifi_status.sta_ssid, wifi_status.sta_ip);
            }

            if (can_status.running) {
                ESP_LOGI(TAG,
                         "CAN: RX=%lu, TX=%lu, Err=%lu, Running=%s",
                         can_status.rx_count,
                         can_status.tx_count,
                         can_status.errors,
...
// pour éviter les conflits avec la tâche LED/RMT
    ...
    if (ble_init_status == ESP_OK) {
        ...
        if (ble_start_status != ESP_OK) {
            ESP_LOGW(TAG, "Impossible de démarrer le service BLE: %s", esp_err_to_name(ble_start_status));
        }
    } else {
    ...
    create_task_on_general_core(
            can_event_task, "can_event_task", 8192, NULL, 4, NULL);  // Augmenté à 8KB à cause de config_profile_t
