...
static void effect_running_lights(void) {
    ...
    for (int i = 0; i < NUM_LEDS; i++) {
        ...
        uint8_t brightness = current_config.brightness * (NUM_LEDS - distance * 2) / NUM_LEDS;
...
static void effect_scan(void) {
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        // Appliquer la traînée des deux côtés, mais seulement dans les limites du strip
...
static void effect_hazard(void) {
    ...
    if (cycle < animation_duration) {
        // Défilement fluide : allumer progressivement les deux côtés depuis le centre
        int lit_count = (cycle * half_leds) / animation_duration;
...
static void effect_charge_status(void) {
    ...
    if (cycle_length > TRAIL_LENGTH) {
        ...
        // Traînée de la même couleur derrière le pixel (8 pixels pour traînée longue et fluide)
        for (int trail = 1; trail <= TRAIL_LENGTH; trail++) {
            ...
            // 2. Elle n'a pas encore été "avalée" par la barre de charge (>= target_led)
            if (trail_pos >= target_led && trail_pos < NUM_LEDS) {
                // Décroissance exponentielle agressive pour une traînée bien visible
                // trail 1 = 80% (204), trail 2 = 60% (153), trail 3 = 40% (102), trail 4 = 25% (64), trail 5 = 10% (25)
