...
// ---------------------------------------------------------------------------
// Conversion vers format BLE compact
// ---------------------------------------------------------------------------

void vehicle_state_to_ble(const vehicle_state_t *src, vehicle_state_ble_t *dst) {
  if (!src || !dst) return;

  // Conversion des floats en int16 avec facteur 10
  ...
  dst->odometer_km = (uint32_t)src->odometer_km;  // uint32 = max 4 294 967 km
  dst->brightness = (uint8_t)(src->brightness * 100.0f);  // 0-100%

  // Valeurs uint8 directes
  dst->gear = src->gear;
  dst->accel_pedal_pos = src->accel_pedal_pos;
  dst->charge_status = src->charge_status;
  dst->autopilot = src->autopilot;

  // Bit packing des booléens (steering wheel buttons et doors_open_count retirés)
  // Byte 0: doors & locks
  dst->flags0 =
    (src->locked                ? (1<<0) : 0) |
  ...
  // Byte 4: safety 2 & autopilot alerts
  dst->flags4 =
