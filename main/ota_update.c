...
esp_err_t ota_begin(size_t total_size)
{
    ...
    ESP_LOGI(TAG, "Début OTA vers partition: %s à 0x%08lx", update_partition->label, update_partition->address);
    ...
    if (ret != ESP_OK) {
        ...
        snprintf(current_progress.error_msg,
                 sizeof(current_progress.error_msg),
                 "Erreur démarrage OTA: %s",
                 esp_err_to_name(ret));
        current_progress.state = OTA_STATE_ERROR;
        led_effects_stop_progress_display();
        led_effects_show_upgrade_error();
...
esp_err_t ota_end(void)
{
    ...
    if (ret != ESP_OK) {
        if (ret == ESP_ERR_OTA_VALIDATE_FAILED) {
            ...
            strncpy(current_progress.error_msg,
                    "Validation du firmware échouée",
